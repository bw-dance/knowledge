...
![image-20221031091256866](https://mynotepicbed.oss-cn-beijing.aliyuncs.com/img/image-20221031091256866.png)

**业务处理（入队前）**

**注意：**所有redis的操作都是在同一yearId的命名空间下。此命名空间下，创建不同操作的命名空间并创建对应的key。

1. 报名的时候，用户传来课程id，去redis获取课程详情以及校验课程的选课时间，出现异常则直接return。
2. 加redission的可重入锁，锁后续操作。
   1. 目的：解决一名学生，同时发送多条不同课程的报名请求，导致数量判断失效，选多门课程问题。
   2. 使用该锁的原因：锁过期会自动续期，防止程序还没运行完，锁过期的现象。
   3. key1:yearId_studentId
   4. value:1/0
3. redis使用map数据结构存储当前学生的选课信息（过期时间为选课周期）
   1. key1：yearId_studentId
   2. key2：courseId
   3. value：1/0
   4. 问题：如果之后的业务出现异常，如程序中断，那么会造成死锁
4. 使用信号量存储课程报名的人数限制（过期时间为选课周期）
   1. key:yearId_courseId
   2. value:num

如果还存在并发下处理慢的问题，那么就是用限流。

**业务处理（入队后）**

1. 消息消费时，如果消息消费失败，不能直接删除redis已选课程和课程限报信息。

   ![image-20221030110539477](https://mynotepicbed.oss-cn-beijing.aliyuncs.com/img/image-20221030110539477.png)

   原因：原则上，投递的报名信息是不可重复的，但是，如果消息队列在处理消息的过程中，发送到交换机的消息没有及时得到回馈（网络延迟等），此时会重新投递消息，那么此时队列中就有了重复消息，如果第一条消费成功，第二条消费失败，删除redis的信息的话，会产生数据的不一致。

   解决方案：针对不同情况，分步骤解决

   ![image-20221031085933047](https://mynotepicbed.oss-cn-beijing.aliyuncs.com/img/image-20221031085933047.png)

#### 取消报名

![image-20221031093810063](https://mynotepicbed.oss-cn-beijing.aliyuncs.com/img/image-20221031093810063.png)

1. 取消报名：
   1. 复用报名时候的分布式锁。
   2. 取消报名时，先删除redis中已选课程的信息，并将redis中课程余量+1。
   3. 将取消报名消息发送到队列，进行数据库的库存恢复。
   4. 做好逻辑判断。
2. 遗留问题：
   1. 三条请求：A报名，B取消报名，C报名
   2. 问题：
      1. rabbitmq并不能完全保证消息的消费顺序，如果B消费时，消费失败，B重入队列，而C在B还未重新入队时，被消费掉了，此时C取消就取消了两次报名信息，造成数据不一致问题。解决方式：只开启一个消费者。
      2. 报名的时候，如果A报名消费失败，但是提示用户的是报名成功，此时消息入的是死信队列，此时并没有实际减数据库库存，如果取消报名，那么又把库存加回来，相当于多加了一次库存。
   3. 上述两个问题的解决访问：
      1. 用户每次报名时，都为用户生成一个当次报名的uuid，并存入redis中，作为key2，取消的时候，拿这个uuid去redis判断是否删除。消费者消费的时候，也拿这个uuid去判断用户存入的报名信息，如果一样则删除报名信息，如果不一样则不进行删除。

#### 查看报名信息

 ![image-20221031104156096](https://mynotepicbed.oss-cn-beijing.aliyuncs.com/img/image-20221031104156096.png)

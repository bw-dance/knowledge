...
5. 基础知识：
   ...
   3. 线程之间同步的方式
      ...
      2. synchronized：保证多个线程互斥访问临界区（共享存储），来实现线程间的通信。 利用wait()、notify()、notifyaAl()方法来实现多个线程互斥访问临界区资源，bject类这几个方法必须配合synchronized来进行使用。
   ...
   7. 世界杯转播期间，如果要设计一个转播系统，你会使用tcp还是udp
      1. 转播期间，对数据要求并不是特别高，允许一定的丢包（面试官说的）

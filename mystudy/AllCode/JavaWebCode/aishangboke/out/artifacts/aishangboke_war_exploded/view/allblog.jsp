...
<body>
<header>
    <h1 class="title">爱尚博客联盟</h1>
</header>
<article class="article">
    <form id="from-table" method="post" action="/aishangboke/deleteSelectServlet?source=allBlog">
        <%--        博客表头--%>
            ...
            <table class="layui-table">
                ...
                <c:forEach items="${pageSearch.blogs}" var="blog" varStatus="s">
                    <tr>
                        ...
                        <td>${blog.author}</td>
                        <td>${blog.upDate}</td>
                        <td><a href="/aishangboke/lookBlogServlet?id=${blog.id}&state=look&now=allblog">查看博客</a></td>
                            <%--                        单条记录的删除修改--%>
                        <td>
<%--                            <button type="button" class="layui-btn"><a href="/inputBlogToMeServlet?id=${blog.id}">引入</a></button>--%>
                            <button type="button" class="layui-btn" id="delete-button"  >
                                    <%--                                <a href="javascript:deleteMessage()">删除</a>--%>
<%--                                如果珊瑚的blog的idcard和登陆者的idcard一样，可以进行删除--%>
                        <c:if test="${blog.author==sessionScope.user.idcard}">
                            <a href="/aishangboke/deleteBlogServlet?id=${blog.id}&page=${pageSearch.page}&now=allblog" class="delete-blog">删除</a>
                        </c:if>
<%--                                        如果不一样--%>
                                        <c:if test="${blog.author!=sessionScope.user.idcard}">
                                            <a href="javascript:void(0);" class="delete-other">删除</a>
                                        </c:if>
        ...
        <div class="button-array">
            <div class="layui-btn-group">
                <button type="button" class="add-number layui-btn layui-btn-sm add-number">
                    <a class="layui-icon" href="/aishangboke/pageSearchAllServlet?page=${pageSearch.page-1>0?pageSearch.page-1:1}">&nbsp<&nbsp</a>
                </button>
                <c:forEach begin="1" end="${pageSearch.totalPage<=9?pageSearch.totalPage:10}" var="b" step="1" varStatus="s">
                    <button type="button" class="add-number layui-btn layui-btn-sm">
                        <a href="${pageContext.request.contextPath}/pageSearchAllServlet?page=${b}" class="layui-icon">${b}</a>
                    </button>
                </c:forEach>
                <button type="button" class="add-number layui-btn layui-btn-sm">

                    <a class="layui-icon" href="/aishangboke/pageSearchAllServlet?page=${pageSearch.page+1<=pageSearch.totalPage?pageSearch.page+1:1}">&nbsp>&nbsp</a>
